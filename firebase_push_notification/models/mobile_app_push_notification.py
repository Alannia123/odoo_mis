...
class MobileAppPushNotification(models.Model):
    ...
    if not firebase_admin._apps:
        cred = credentials.Certificate("/home/moses/workspace/odoo_mis/firebase_push_notification/data/service-account-file.json")
    ...
    def send_notification(self):
        ...
        if self.send_notification_to == 'to_all':
            ...
            for rec in self.partner_ids:
                print('lossssssssssssseeeeeeee', rec.mail_firebase_tokens)
                for line in rec.mail_firebase_tokens:
                    # tokenList.append('')
                   print('losssssssssssss',line)
                   tokens.append({'partner_id': line.partner_id.id, 'token':line.token})
